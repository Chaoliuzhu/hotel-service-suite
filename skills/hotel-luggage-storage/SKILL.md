...
description: 酒店行李寄存全流程管理——寄存登记、条码打印、电子凭证发送、取件核销、超期预警。新增"随手拍"AI拍照识别和"快递站模式"存取流程。支持热敏标签打印机、飞书卡片/短信/微信三渠道电子凭证、多维表格表单兜底入口。
触发词：寄存行李、行李寄存、寄存登记、取行李、行李领取、行李条码、行李标签、行李牌、寄存凭证、行李超期、行李查询、行李台账、随手拍、拍照寄存、快递站模式、扫码取件
version: 3.0.0
---
...
"随手拍"是本系统新增的核心功能。当酒店员工在飞书中向 BOT 发送一张行李照片时，AI 自动识别物品信息并预填登记表，大幅减少手动输入。

### 工作流程

```
员工拍照 → 发送给飞书BOT → AI分析照片 → 提取物品信息 → 生成确认卡片 → 员工确认 → 写入台账
```
...
| 字段 | 说明 | 示例 |
|------|------|------|
| 物品名称 | 行李类型 | 行李箱、背包、手提袋、纸箱 |
| 物品类别 | 分类 | 普通行李/贵重行李/大件箱包/易碎物品 |
| 详细描述 | 颜色、尺寸、特征 | 黑色28寸Rimowa行李箱，有贴纸 |
| 主要颜色 | 外观颜色 | 黑色 |
| 品牌 | 如可识别 | Rimowa、Samsonite |
| 物品状态 | 外观评估 | 全新/九成新/有使用痕迹/破损 |
| 贵重标记 | 是否贵重 | 是/否 |
| 置信度 | AI识别准确度 | 0.0 ~ 1.0 |

### 确认卡片

AI 识别完成后，系统自动发送飞书交互卡片，包含:
- AI 识别结果（含置信度百分比）
- 宾客信息区域（姓名、房号、电话）
- **确认寄存** 按钮：直接写入台账
- **需要修改** 按钮：引导补充信息
...
**重要: 拍照仅在存入环节，取出环节无需拍照。**

| 环节 | 是否拍照 | 说明 |
|------|----------|------|
| 存入（寄存） | 必须拍照 | AI 识别物品信息，生成条码标签和凭证 |
| 取出（取件） | 无需拍照 | 仅需扫码/报号 + 验证码即可取走 |
...
借鉴菜鸟驿站/丰巢的物品存取流程，核心原则: **存入拍照、取出扫码**。

| 环节 | 快递站 | 酒店行李 |
|------|--------|----------|
| 存入 | 扫码→拍照→上架→发短信 | 拍照→AI识别→贴标签→发凭证 |
| 取出 | 报取件码→扫码验证→取走 | 报编号/扫码→验证码→取走 |
...
# result.number  -> "LG-20260623-001"
# result.pickup_code -> "5864"
# result.qr_path -> "/tmp/hotel-qr/LG-20260623-001.png"
```
...
result = processor.pickup_item(
    scan_code_or_number="LG-20260623-001",
    verification_code="5864",
    claimer="张三",
)

# 方式二: 扫码 (二维码内含编号和验证码)
result = processor.verify_pickup(
    scan_data="LG-20260623-001|5864",
)
```

### QR 码取件

系统为每件寄存行李自动生成二维码，内容格式: `LG-20260623-001|5864`（编号|验证码）。
员工使用手机或扫码枪扫描标签上的二维码，即可自动触发取件流程。
...
当 AI BOT 不可用时（网络故障、API 限流等），员工可通过多维表格的"收集表单"视图手动填写。
表单字段与主台账完全一致，提交后自动写入主表，无需额外同步。
...
3. 员工在 AI 不可用时直接打开链接填写
4. 提交后数据自动写入多维表格主台账

### 双入口架构总结

```
入口A (AI智能): 拍照发BOT → AI识别 → 确认卡片 → 自动录入 → 打印标签 → 发凭证
入口B (表单兜底): 多维表格表单 → 手动填写 → 自动同步 → 打印标签 → 发凭证
```

两个入口最终写入同一张多维表格，数据完全一致。
